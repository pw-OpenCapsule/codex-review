...
SCHEMA={'type':'object','properties':{'issues':{'type':'array','items':{'type':'object','properties':{
 'severity':{'type':'string','enum':['P0','P1','P2']},'summary':{'type':'string'},'file':{'type':'string'},
...
def main():
 ...
 with Codex(config=CodexConfig(codex_bin=shutil.which('codex'),cwd=str(cwd))) as codex:
  thread=codex.thread_start(cwd=str(cwd),sandbox=Sandbox.read_only,approval_mode=ApprovalMode.deny_all,
       developer_instructions=RULES,ephemeral=True,config=cfg,model=os.environ.get('CODEX_REVIEW_MODEL','gpt-6-astra'))
  result=thread.run(prompt,output_schema=SCHEMA,effort=os.environ.get('CODEX_REVIEW_EFFORT','low'))
