...
def policy_digest(config):
    ...
    files=('review.py','routing.py','focused.py','budget.py','model_profile.py','reuse.py')
    h=hashlib.sha256()
    for name in files:h.update(name.encode()+b'\0'+(root/name).read_bytes())
    h.update(json.dumps({k:config.get(k) for k in ('routing','model','effort','codex_bin')},sort_keys=True).encode())
    return h.hexdigest()
