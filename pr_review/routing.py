...
只允许使用提供的差异预览和上下文，不调用任何工具、不自行读文件。局部信息足够时直接完成；预览截断时先申请有限片段；补齐后只有具体复杂推理需求才交给深度阶段，不能把截断本身作为升级证据，不能自行展开。避免为了分流就做一遍昂贵的完整深度审查。summary 简述已看范围、已确认约束与尚待核实内容，供下一阶段复用。'''
