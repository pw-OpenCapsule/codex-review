#!/usr/bin/env python3
"""git blame 一段行范围，输出 author info + 可选映射到 lark open_id.

Usage:
  blame_lookup.py --workdir <repo> --file <path> --line-start <n> --line-end <n>
                  [--user-map <tsv>] [--default-lark <open_id>]
"""
...
def main():
    ...
    ap.add_argument("--default-lark", default="")
    args = ap.parse_args()
    ...
    try:
        ...
        print(json.dumps({"author_email": "", "author_name": "",
                          "sha": "", "date": "", "lark_open_id": args.default_lark}))
        return
    ...
    lark_open_id = args.default_lark
    if args.user_map and os.path.exists(args.user_map) and author_email:
        lark_open_id = lookup_user_map(args.user_map, author_email) or args.default_lark
